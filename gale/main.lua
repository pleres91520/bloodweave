...
local gabriel2Type = Isaac.GetPlayerTypeByName("Gabriel2", false) -- Exactly as in the xml. The second argument is if you want the Tainted variant.
local hairCostume = Isaac.GetCostumeIdByPath("gfx/characters/gabriel2_hair.anm2") -- Exact path, with the "resources" folder as the root
...
function MyCharacterMod:GiveCostumesOnInit(player)
    if player:GetPlayerType() ~= gabriel2Type then
        return -- End the function early. The below code doesn't run, as long as the player isn't Gabriel2.
    end
...
local SPEED_ADDITION = 0.13 --初始值1
local TEARS_REDUCTION = 0 --初始值2.73（此值越大，射速越低）
local VELOCITY_ADDITION = 0 --初始值1（弹速）
