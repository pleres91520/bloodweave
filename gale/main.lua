...
local GaleType = Isaac.GetPlayerTypeByName("Gale", false) -- Exactly as in the xml. The second argument is if you want the Tainted variant.
local hairCostume = Isaac.GetCostumeIdByPath("gfx/characters/Gale_hair.anm2") -- Exact path, with the "resources" folder as the root
...
function MyCharacterMod:GiveCostumesOnInit(player)
    if player:GetPlayerType() ~= GaleType then
        return -- End the function early. The below code doesn't run, as long as the player isn't Gale.
    end
