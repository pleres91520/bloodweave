...
local AstarionType = Isaac.GetPlayerTypeByName("Astarion", false) -- Exactly as in the xml. The second argument is if you want the Tainted variant.
local hairCostume = Isaac.GetCostumeIdByPath("gfx/characters/Astarion_hair.anm2") -- Exact path, with the "resources" folder as the root
...
function MyCharacterMod:GiveCostumesOnInit(player)
    if player:GetPlayerType() ~= AstarionType then
        return -- End the function early. The below code doesn't run, as long as the player isn't Astarion.
    end
...
local DAMAGE_REDUCTION = 1.5 --初始值3.5（此值越大，伤害越低）
local LUCK_ADDITION = 1 --初始值0
local SPEED_ADDITION = 0.15 --初始值1
local TEARS_REDUCTION = 0.3 --初始值2.73（此值越大，射速越低）
local VELOCITY_ADDITION = 0.2 --初始值1（弹速）
