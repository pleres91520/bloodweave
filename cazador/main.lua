...
local CazadorType = Isaac.GetPlayerTypeByName("Cazador", false) -- Exactly as in the xml. The second argument is if you want the Tainted variant.
local hairCostume = Isaac.GetCostumeIdByPath("gfx/characters/Cazador_hair.anm2") -- Exact path, with the "resources" folder as the root
...
function MyCharacterMod:GiveCostumesOnInit(player)
    if player:GetPlayerType() ~= CazadorType then
        return -- End the function early. The below code doesn't run, as long as the player isn't Cazador.
    end
